...
make sync-version
# or
python scripts/sync_version.py --entry udi-mqtt-pg3x.py
```

Or bump in one step:

```bash
python scripts/bump_version.py 0.51.0 --entry udi-mqtt-pg3x.py
```
...
**Do not** change controller node addresses (`mqctrl`) on shipped plugins.
...
```bash
cd ~/polyglot/nodeservers/udi-mqtt-pg3x
make install-eisy UV_PYTHON="$UV_PYTHON"
direnv allow .
```

MQTT has lighter runtime deps than Hunter/TaHoma; Polyglot runtime is unchanged — still `install.sh` + system `pip`, not uv.

## server.json GitHub URLs

| Stage | Org | Repo |
|-------|-----|------|
| Development | `sejgit-udi-plugins` | `udi-mqtt-pg3x` |
| Published on UDI store | `UniversalDevicesInc-PG3` | `udi-mqtt-pg3x` |
